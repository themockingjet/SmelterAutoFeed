...
PERFORMANCE_LOGGING_DEFAULT="${PERFORMANCE_LOGGING_DEFAULT:-false}"
...
ARCHIVE_PATH="${ARCHIVE_PATH:-$RELEASE_DIR/${PACKAGE_NAME}-${PACKAGE_VERSION}${PACKAGE_SUFFIX}.zip}"
if [[ "$ARCHIVE_PATH" != /* ]]; then
	ARCHIVE_PATH="$PROJECT_ROOT/$ARCHIVE_PATH"
fi
make validate-release

PERFORMANCE_LOGGING_DEFAULT="$PERFORMANCE_LOGGING_DEFAULT" \
BUILD_CONFIGURATION="$BUILD_CONFIGURATION" \
RELEASE_OUTPUT_PATH="$RELEASE_DIR" \
./scripts/build.sh
